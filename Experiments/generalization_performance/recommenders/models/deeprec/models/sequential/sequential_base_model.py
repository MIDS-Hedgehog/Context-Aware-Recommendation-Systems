...
class SequentialBaseModel(BaseModel):
    ...
    def fit(
        ...
        for epoch in range(1, self.hparams.epochs + 1):
            ...
            if os.path.exists(path_epocs_losses):
                with open(path_epocs_losses, "rb") as file:
            ...
            if self.hparams.save_model and self.hparams.MODEL_DIR:
                ...
                if progress:
                    checkpoint_path = self.saver.save(
                        sess=train_sess,
                        save_path=self.hparams.MODEL_DIR
                        + model_name  # LEONARD
                        + "_epoch_"  # LEONARD
                        + str(epoch),
                    )
                    checkpoint_path = self.saver.save(  # noqa: F841
                        ...
                        save_path=os.path.join(
                            self.hparams.MODEL_DIR,
                            model_name + "_best_model",  # LEONARD
                        ),
                    )
        ...
        return self, epoch_losses  # LEONARD
