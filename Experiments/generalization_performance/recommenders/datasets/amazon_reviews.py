...
def data_preprocessing(
    ...
    # LEONARD
    sampled_instance_file = None
    if selection_type == "random":
        sampled_instance_file = _get_sampled_data(
            instance_output, sample_rate=sample_rate
        )
    elif selection_type == "user":
        sampled_instance_file = _get_user_data(instance_output, sample_rate=sample_rate)
    elif selection_type == "time":
        sampled_instance_file = _get_time_data(instance_output, sample_rate=sample_rate)
...
# LEONARD
def _get_user_data(instance_file, sample_rate):
    ...
    unique_users = ns_df["user_id"].unique()
    k = int(len(unique_users) * sample_rate)
    selected_users = np.random.choice(unique_users, k, replace=False)
    ns_df_sample = ns_df[ns_df["user_id"].isin(selected_users)]
...
# LEONARD
def _get_time_data(instance_file, sample_rate):
    logger.info("getting sampled data...")
    global item2cate
    output_file = instance_file + "_" + str(sample_rate)
    columns = ["label", "user_id", "item_id", "timestamp", "cate_id"]
...
def _meta_preprocessing(meta_readfile):
    ...
    with open(meta_readfile, "r") as meta_r, open(meta_writefile, "w") as meta_w:
        for line in meta_r:
            try:
            ...
            # Use 'asin' if available; otherwise fall back to 'parent_asin'
            asin = line_new.get("asin") or line_new.get("parent_asin", "unknown")

            # Prefer main_category; if not available, then check for categories.
            if line_new.get("main_category"):
                category = line_new.get("main_category")
            else:
                categories = line_new.get("categories")
                if isinstance(categories, list) and len(categories) > 0:
                    category = categories[-1]
                else:
                    category = "unknown"

            meta_w.write(asin + "\t" + category + "\n")
...
def _reviews_preprocessing(reviews_readfile):
    ...
    with open(reviews_readfile, "r") as reviews_r, open(
        reviews_writefile, "w"
    ) as reviews_w:
        for line in reviews_r:
            ...
            # Use reviewerID if available; otherwise, fallback to user_id
            if "reviewerID" in review:
                reviewer = review["reviewerID"]
                timestamp = review.get("unixReviewTime")
            elif "user_id" in review:
                reviewer = review["user_id"]
                timestamp = review.get("unixReviewTime") or review.get("timestamp")
            else:
                logger.error("No valid reviewer field found in review")
                continue

            # Determine parent_asin: use the provided parent_asin if available; otherwise, use asin
            parent_asin = review.get("parent_asin", review.get("asin"))

            # Ensure required key "asin" exists
            if "asin" not in review:
                logger.error("Missing asin in review")
                continue

            # Write out reviewer, asin, parent_asin, and timestamp
            reviews_w.write(
                str(reviewer)
                + "\t"
                + str(review["asin"])
...
def _create_instance(reviews_file, meta_file):
    ...
    # Read reviews: each line in reviews_file is expected to have 4 tab-separated fields:
    # [reviewer, asin, parent_asin, timestamp]
    f_reviews = open(reviews_file, "r")
    user_dict = {}
    for line in f_reviews:
        line = line.strip()
        fields = line.split("\t")
        # fields: [reviewer, asin, parent_asin, timestamp]
        user = fields[0]  # reviewer ID
        try:
            ts = float(fields[3])
        except Exception:
            ts = 0
        if user not in user_dict:
            user_dict[user] = []
        user_dict[user].append((line, ts))
    f_reviews.close()

    # Build meta dictionary: key -> asin, value -> category (as produced by _meta_preprocessing)
    f_meta = open(meta_file, "r")
    meta_dict = {}
    for line in f_meta:
        line = line.strip()
        meta_fields = line.split("\t")
        if meta_fields[0] not in meta_dict:
            meta_dict[meta_fields[0]] = meta_fields[1]
    f_meta.close()

    f_output = open(output_file, "w")
    for user, reviews in user_dict.items():
        sorted_reviews = sorted(reviews, key=lambda x: x[1])
        for line, ts in sorted_reviews:
            fields = line.split("\t")
            reviewer = fields[0]  # user ID
            asin = fields[1]  # item ID
            parent_asin = fields[2]  # for lookup fallback
            timestamp = fields[3]
            # Look up category: first try asin, then parent_asin.
            if asin in meta_dict:
                category = meta_dict[asin]
            elif parent_asin in meta_dict:
                category = meta_dict[parent_asin]
            else:
                category = "unknown"
            # Output in the expected order: label, user_id, item_id, timestamp, category
            f_output.write(
                "1\t"
                + reviewer
                + "\t"
...
def _download_reviews(name, dest_path):
    ...
    url = (
        "https://mcauleylab.ucsd.edu/public_datasets/data/amazon_2023/raw/"  # LEONARD
        + (
            # LEONARD
            "meta_categories/"
            if "meta_" in name.lower()
            else "review_categories/"
        )
        + name
        + ".gz"
    )
